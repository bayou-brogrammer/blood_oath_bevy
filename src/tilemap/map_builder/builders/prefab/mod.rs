...
impl PrefabBuilder {
    ...
    fn char_to_map(&mut self, ch: char, idx: usize, builder: &mut MapBuilder) {
        match ch {
            ...
            '#' => *builder.map.tiles.get_index_checked_mut(idx) = TileType::Wall,
            '@' => {
                let pt = builder.map.index_to_point2d(idx);
                *builder.map.tiles.get_index_checked_mut(idx) = TileType::Floor;
                builder.starting_position = Some(pt.to_coord());
            }
            '>' => *builder.map.tiles.get_index_checked_mut(idx) = TileType::DownStairs,
            'g' => {
                *builder.map.tiles.get_index_checked_mut(idx) = TileType::Floor;
    ...
    fn apply_sectional(
        ...
        let chunk_x = match section.placement.0 {
            ...
            HorizontalPlacement::Center => {
                (builder.map.width() / 2) - (section.width as i32 / 2)
            }
        ...
        let chunk_y = match section.placement.1 {
            ...
            VerticalPlacement::Center => {
                (builder.map.height() / 2) - (section.height as i32 / 2)
            }
    ...
    fn apply_room_vaults(&mut self, builder: &mut MapBuilder, rng: &RandomNumbers) {
        use prefab_rooms::*;
        ...
        let vault_roll = rng.roll_dice(1, 6) + builder.map.depth;
        if vault_roll < 4 {
            return;
        }
        ...
        let mut possible_vaults: Vec<&PrefabRoom> = master_vault_list
            .iter()
            .filter(|v| {
                builder.map.depth >= v.first_depth && builder.map.depth <= v.last_depth
            })
            .collect();
