...
impl BspDungeonBuilder {
    ...
    fn get_random_rect(&mut self, rng: &RandomNumbers) -> Rect {
        if self.rects.len() == 1 {
            return self.rects[0];
        }
    ...
    fn get_random_sub_rect(&self, rect: Rect, rng: &RandomNumbers) -> Rect {
        let mut result = rect;
        ...
        for y in expanded.y1..=expanded.y2 {
            for x in expanded.x1..=expanded.x2 {
                ...
                if can_build {
                    ...
                    if *builder.map.tiles.get_index_checked(idx) != TileType::Wall {
                        can_build = false;
                    }
                }
            }
