...
impl RoomCornerRounder {
    ...
    fn fill_if_corner(&mut self, x: i32, y: i32, builder: &mut MapBuilder) {
        ...
        if x > 0 && *builder.map.tiles.get_index_checked(idx - 1) == TileType::Wall {
            neighbor_walls += 1;
        }
        if y > 0 && *builder.map.tiles.get_index_checked(idx - w as usize) == TileType::Wall {
            neighbor_walls += 1;
        }
        if x < w - 2 && *builder.map.tiles.get_index_checked(idx + 1) == TileType::Wall {
            neighbor_walls += 1;
        }
        if y < h - 2
            && *builder.map.tiles.get_index_checked(idx + w as usize) == TileType::Wall
        {
            neighbor_walls += 1;
        }
