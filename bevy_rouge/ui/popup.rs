...
fn popup_ui(mut commands: Commands, turn_state: Res<State<TurnState>>, fonts: Res<FontAssets>) {
                            ...
                            parent
                                .spawn_bundle(TextBundle {
                                    style: Style {
                                        size: Size::new(
                                            Val::Auto,
                                            Val::Px(20. * (INVENTORY_SLOTS + 1) as f32),
                                        ),
                                        margin: UiRect {
                                            left: Val::Auto,
                                            right: Val::Auto,
...
impl Plugin for PopUpPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<ChosenItemEvent>()
            ...
            .add_system_set(SystemSet::on_update(TurnState::InventoryPopup).with_system(player_input))
            .add_system_set(SystemSet::on_update(TurnState::EquipmentPopup).with_system(player_input))
            // cleanup when exiting
            .add_system_set(SystemSet::on_exit(TurnState::InventoryPopup).with_system(despawn_menu))
            .add_system_set(SystemSet::on_exit(TurnState::EquipmentPopup).with_system(despawn_menu))
            .add_system_set(SystemSet::on_enter(TurnState::InventoryPopup).with_system(popup_ui))
            .add_system_set(SystemSet::on_enter(TurnState::EquipmentPopup).with_system(popup_ui));
    }
}
