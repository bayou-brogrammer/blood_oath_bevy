...
fn update_inventory_text(
    ...
    if items_query.is_empty() {
        ...
        text.sections[0].value = "You have no items.".to_string();
        description.sections[0].value = " ".to_string();
    } else {
        items_query
            .iter()
            ...
            .for_each(|(index, (_, item, desc, _))| {
                ...
                if index as i32 == highlighted_item.0 {
                    mark = "-";
                    description.sections[0].value = desc.0.to_string();
                } else {
                    mark = " ";
                }
                // update text
...
fn use_item(
    ...
    if let Some(item_entity) = item_entity {
        commands.spawn().insert(ActivateItem { used_by: player_ent, item: item_entity });
