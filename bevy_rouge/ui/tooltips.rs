...
fn tooltip_ui(mut commands: Commands, fonts: Res<FontAssets>) {
    commands
            ...
            parent
                .spawn_bundle(TextBundle {
                    ...
                    text: Text::from_section(
                        "Goblin. HP: 2 / 2",
                        TextStyle { font: fonts.fira_sans.clone(), font_size: 20.0, color: Color::WHITE },
                    ),
                    ..Default::default()
                })
                .insert(ToolTipText);
...
fn update_tooltip(
    ...
    q_names: Query<(&Naming, &Point, Option<&Health>)>,
    ...
    if buttons.just_pressed(MouseButton::Left) {
        ...
        if let Some(pos) = wnd.cursor_position() {
            ...
            let grid_position = Point { x: grid_x as i32, y: grid_y as i32 };
            ...
            q_names
                .iter()
                .filter(|(_, pos, _)| **pos == grid_position && player_fov.visible_tiles.contains(*pos))
...
impl Plugin for TooltipsPlugin {
    fn build(&self, app: &mut App) {
        // app.add_system_set(SystemSet::on_exit(TurnState::MainMenu).with_system(tooltip_ui))
        //     .add_system_set(SystemSet::on_update(TurnState::AwaitingInput).with_system(update_tooltip))
        //     .add_system_set(SystemSet::on_exit(TurnState::AwaitingInput).with_system(hide_tooltip));
    }
}
