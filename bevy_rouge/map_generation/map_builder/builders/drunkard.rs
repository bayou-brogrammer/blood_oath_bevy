...
impl DrunkardsWalkBuilder {
    ...
    fn build(&mut self, builder: &mut MapBuilder, rng: &mut RandomNumbers) {
        ...
        let starting_position = Point { x: builder.map.width / 2, y: builder.map.height / 2 };
        let start_idx = builder.map.xy_idx(starting_position.x, starting_position.y);
        builder.map.tiles[start_idx] = TileType::Floor;

        let total_tiles = builder.map.width * builder.map.height;
        let desired_floor_tiles = (self.settings.floor_percent * total_tiles as f32) as usize;
        let mut floor_tile_count = builder.map.tiles.iter().filter(|a| **a == TileType::Floor).count();
        ...
        while floor_tile_count < desired_floor_tiles {
            ...
            while drunk_life > 0 {
                ...
                builder.paint(self.settings.symmetry, self.settings.brush_size, drunk_x, drunk_y);
            ...
            floor_tile_count = builder.map.tiles.iter().filter(|a| **a == TileType::Floor).count();
        }
    }
}
