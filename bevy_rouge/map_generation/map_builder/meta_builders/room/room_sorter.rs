...
impl RoomSorter {
    ...
    fn sorter(&mut self, builder: &mut MapBuilder) {
        match self.sort_by {
            RoomSort::Leftmost => builder.rooms.as_mut().unwrap().sort_by(|a, b| a.x1.cmp(&b.x1)),
            RoomSort::Rightmost => builder.rooms.as_mut().unwrap().sort_by(|a, b| b.x2.cmp(&a.x2)),
            RoomSort::Topmost => builder.rooms.as_mut().unwrap().sort_by(|a, b| a.y1.cmp(&b.y1)),
            RoomSort::Bottommost => builder.rooms.as_mut().unwrap().sort_by(|a, b| b.y2.cmp(&a.y2)),
            RoomSort::Central => {
